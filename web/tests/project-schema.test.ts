...
describe("multi-airstream project with cross-stream coupling", () => {
        ...
        airstreams: [
          {
            ...
            stages: [
              ...
              },
              {
                ...
                name: "Enthalpy wheel",
                params: {
                  effectivenessSensible: 0.75,
                  effectivenessLatent: 0.68,
                },
                couplings: [
                  {
                    role: "exchange-stream",
                    airstreamId: "exhaust",
                    stageId: "ra",
                  },
                ],
  ...
  it("expresses every coupling role the process set needs", () => {
    ...
    for (const role of roles) {
      ...
      withRole.systems[0]!.airstreams[0]!.stages[1]!.couplings = [
...
describe("schema rejects malformed projects", () => {
  it("rejects an unknown schema version", () => {
    const bad = { ...emptyProject(), schemaVersion: SCHEMA_VERSION + 1 };
    expect(validate(bad)).toBe(false);
  });

  it("rejects a missing atmosphere", () => {
    const bad = structuredClone(emptyProject()) as unknown as Record<
      string,
      unknown
    >;
  ...
  it("rejects a project with no systems", () => {
    const bad = { ...emptyProject(), systems: [] };
    expect(validate(bad)).toBe(false);
  });

  it("rejects a system with no airstreams", () => {
    const bad = structuredClone(emptyProject()) as Project;
    bad.systems[0]!.airstreams = [];
    expect(validate(bad)).toBe(false);
  });

  it("rejects a system role it does not know", () => {
    const bad = structuredClone(emptyProject()) as Project;
    bad.systems[0]!.role = "dehumidification" as never;
    expect(validate(bad)).toBe(false);
  });

  it("rejects an unknown stage type", () => {
    const bad = structuredClone(emptyProject()) as Project;
    bad.systems[0]!.airstreams[0]!.stages = [
      { id: "x", type: "teleporter" as never },
    ];
    expect(validate(bad)).toBe(false);
  });
  ...
  it("rejects an id with illegal characters", () => {
    const bad = structuredClone(emptyProject()) as Project;
    bad.systems[0]!.airstreams[0]!.id = "has spaces";
    expect(validate(bad)).toBe(false);
  });
